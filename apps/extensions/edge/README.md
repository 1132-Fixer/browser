...
## How this build differs from the Chrome build

The code is identical. Only the manifest `name` and `description` differ, because Microsoft's
certification requires that a package listed on Microsoft Edge Add-ons does not brand itself as a
Chrome extension. There is no `update_url`; Microsoft Edge Add-ons owns updates.
...
version, run `npm ci` and `npm run build:edge` with the Node version in `.nvmrc`, then compare
`dist/edge/` with this folder.
